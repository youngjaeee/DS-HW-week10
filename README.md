...
After deadline, I found some errors occured when trying to delete root node.
So I'm adding the edited source codes here.
I uploaded the codes here just as reference for the next time.
...
int deleteNode(Node* head, int key) // 입력한 key값과 동일한 값을 가지는 노드를 지우는 함수
{
	if (isintialized == 0)
	{
		printf("BST 초기화를 진행하지 않았습니다. 초기메뉴에서 z를 눌러 초기화하십시오.\n");
		return;
	}
	...
	if (head->left == ptr)
	{
		if (head->left->left == NULL && head->left->right == NULL) // 노드 root 1개만 존재
		{
			head->left = NULL;
			free(ptr);
			return 0;
	...
	{
		if (preptr->right == preptr) // ptr이 root을 가리킬 경우
		{
			head->left = ptr->left;
			ptr->left = NULL;
			ptr->right = NULL;
